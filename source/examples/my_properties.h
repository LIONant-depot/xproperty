...
//------------------------------------------------------------------------------
// CORE INCLUDES
// Add here all your core containers and types for your xproperty library
//------------------------------------------------------------------------------
#include<string>
#include<vector>
#include<map>
...
{
    // The worse case scenario for a single xproperty with a 8 byte alignment
    // Basically your biggest type. A simple way to do it is to use std::max
    // to determine your worse case, if you are not sure on some of your types
    // then simply find the aligned multiple of 8 bytes. Here is a convenient
    // template to do that for you.
    template< typename...T>
    inline constexpr std::size_t max_sizeof_v = (std::max({sizeof(T)...}) + 7) / 8;

    // Now we declare the worse case scenario for a single xproperty with a 8 byte alignment
    // We use the standard array as a simple way to do it.
    using data_memory = std::array
    < std::uint64_t
    , max_sizeof_v
    ...
    // The worse case scenario for a single iterator with a 8 byte alignment
    // We use the standard array as a simple way to do it.
    using iterator_memory = std::array
    < std::uint64_t
    , max_sizeof_v
        < std::vector<std::string>::iterator
        , std::map<std::string, std::string>::iterator
        , std::uint64_t
        , std::array<std::uint64_t, 4>   // you could set a minimum size here if you want
        >
    >;
}
...
// We add here the rest of the configuration for the xproperty library
//------------------------------------------------------------------------------
namespace xproperty::settings
{
    //
    ...
    //
    // We configure all our atomic types here
    //
    template<>
    ...
    // maximum 2 levels of pointers. If you need more you can add them
    //
    template<typename T>
    ...
    //
    // We can also add support for smart pointers
    //
    ...
    //
    // We can also support C-array as well. We added up to 3D arrays,
    ...
    //
    // We add some random stupid container here from scratch to serve as an example
    ...
    //
    // We add the definition of our stupid container here...
    //
    template< typename T >
...
//
// We put what the user needs to use in these name space
// to minimize the work for the user
//
namespace xproperty
{
